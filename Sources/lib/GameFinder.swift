...
actor GameFinder {
  ...
  func findGames(showProgress: Bool = false) async throws {
    ...
    let fourtiles = await FourtileProvider(
      fourtiles: words.words.filter { Self.validWordLengths.contains($0.count) }
    )
  ...
  private func findGame(forWords fourtiles: [String]) async -> Game? {
    ...
    guard await possibleFourtiles(inTiles: tiles).count == Self.numFourtilesPerGame else {
      return nil
    }
  ...
  private func possibleFourtiles(inTiles tiles: Set<String>) async -> Set<String> {
    let possibleArrangements = tiles.permutations(ofCount: Self.numTilesPerFourtile).map {
      $0.joined()
    }
